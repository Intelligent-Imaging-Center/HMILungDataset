...
# data_source_folder = "D:/lin1TestField/data_source"
...
# 定义暗背景图片，输出文件夹
output_datacube_folder = 'D:/Training4/DataSource/data/x-sg-x'
no_light_background_path = 'D:/lin1TestField/dark1.tiff'
    ...
    for slide_folder in parent_folders:
        ...
        for area_folder in area_folders:
            ...
            for i in range(band_min, band_max, step):
                ...
                reflectance[reflectance >= 2] = 1.99999
            ...
            spectral_cube = np.clip(spectral_cube, 0, 2)
            spectral_cube = spectral_cube/2*255
            spectral_cube = np.uint8(spectral_cube)
            # print("cliped max is ", np.max(spectral_cube), " min is ", np.min(spectral_cube))
