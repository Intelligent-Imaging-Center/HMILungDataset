...
for f in file_list:
    ...
    source_label = read_tif_img(os.path.join(source_dir,filename_ext))
    h,w = source_label.shape
    target_label = np.zeros((3088,2064),dtype = np.uint8)
    target_label[:h,:w] = source_label
    im = Image.fromarray(target_label)
