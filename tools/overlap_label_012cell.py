...
def process_tif_img(img):
    print("img has unique value ", np.unique(img))
...
ill_label_dir = "../../Training5/ROI-cell-ill/ill-haosong"
cell_label_dir = "../../Training5/ROI-cell-ill/ROI-cell-updated"
target_dir = "../../Training5/ROI-cell-ill/label-updated"
# obtain file list
...
if int(input("Number of files pair to be overlapped is " + str(modifiableNum) + " if correct enter 1\n")) == 1:
    for f in file_list:
        ...
        print(os.path.join(cell_label_dir,filename+".tif"))
        cell_label = read_tif_img(os.path.join(cell_label_dir,filename+".tif"))
        # cell_label = read_process_tif_img(os.path.join(cell_label_dir,filename+".tif"))
        cell_label[cell_label==2]=1
